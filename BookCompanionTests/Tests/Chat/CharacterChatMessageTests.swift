...
final class CharacterChatMessageTests: XCTestCase {
    ...
    func test_userMessageFactory_setsRoleAndContent() async {
        await MainActor.run {
    ...
    func test_characterPlaceholderFactory_isStreamingAndEmpty() async {
        await MainActor.run {
    ...
    func test_safetyBlockFactory_setsSystemKind() async {
        await MainActor.run {
    ...
    func test_safetyEndFactory_setsSystemKind() async {
        await MainActor.run {
    ...
    func test_quotaLimitFactory_setsSystemKind() async {
        await MainActor.run {
    ...
    func test_encode_doesNotIncludeIsStreaming() async throws {
        try await MainActor.run {
            let msg = CharacterChatMessage(role: .character, content: "Hello", isStreaming: true)
    ...
    func test_decode_alwaysSetsIsStreamingFalse() async throws {
        try await MainActor.run {
    ...
    func test_systemKind_roundTrips() async throws {
        try await MainActor.run {
    ...
    func test_nilSystemKind_roundTrips() async throws {
        try await MainActor.run {
